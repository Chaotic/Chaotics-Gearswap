...
--------------------------------------------------------------------------------------------------------------
----
--  This function creates the actual text
----
function construct_HUD_Categories( useLightMode )
	if useLightMode then
		construct_HUD_Categories_Litemode()
	else
		construct_HUD_Categories_NotLitemode()
	end
end

function construct_HUD_Categories_Litemode()
	hud_endofline = ''
...
function construct_HUD_Categories_NotLitemode()
...
--  This function seems to build the hud 
--  Odd name for that I know
...
-- Displays the Hud (why this isn't in the other functions I don't know)
function displayHUD()
...
--------------------------------------------------------------------------------------------------------------
----
-- Set up the text and variables (probably requires the most work for what i want)
----
    ...
    if state.IdleMode  ~= nil then
	    if refreshType ~= nil then
		    if refreshType == "sublimation" then
		        main_text_hud.player_current_idle = tostring(state.IdleMode.value..' + \\cs(32, 255, 32)Sublimation\\cr')
		    else
		        main_text_hud.player_current_idle = state.IdleMode.value
		    end
    	elseif luopanMode ~= nil then
    		if luopanMode == 'pan' then
        		main_text_hud.player_current_idle = tostring(state.IdleMode.value..' + \\cs(32, 255, 32)luopan\\cr')
    		else
        		main_text_hud.player_current_idle = state.IdleMode.value
    		end
    	else
...
function hud_command(commandArgs)
